...
def enlist_subject():
    ...
    try:
        ...
        with get_db_cursor() as (cursor, connection):
            ...
            cursor.execute("SELECT student_id FROM student WHERE USN = %s", (student_id,))
            student = cursor.fetchone()
            if not student:
            ...
            has_conflict, conflict_message = check_time_conflict(cursor, actual_student_id, subj_avail_id)
            if has_conflict:
                return jsonify({
                    'success': False,
...
def check_time_conflict(cursor, student_id, new_subject_id):
