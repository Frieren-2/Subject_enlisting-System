...
// --- Popup Warning Utility ---
function showPopupWarning(msg, timeout = 3500) {
  let popup = document.getElementById('popupWarning');
  let msgSpan = document.getElementById('popupWarningMsg');
  if (!popup) {
    // Create popup if not present
    popup = document.createElement('div');
    popup.id = 'popupWarning';
    popup.className = 'popup-warning';
    popup.style.display = 'none';
    popup.innerHTML = `
      <span class="popup-icon">&#9888;</span>
      <span id="popupWarningMsg"></span>
      <button class="popup-close" onclick="hidePopupWarning()" aria-label="Close">&times;</button>
    `;
    document.body.appendChild(popup);
    msgSpan = popup.querySelector('#popupWarningMsg');
  }
  ...
  if (timeout > 0) {
    ...
    popup.hideTimeout = setTimeout(hidePopupWarning, timeout);
  }
}
...
// --- Modal Popup Utility ---
function showPopupModal(message, callback) {
  let modal = document.getElementById('popupModal');
  if (!modal) {
    modal = document.createElement('div');
    modal.id = 'popupModal';
    modal.className = 'popup-modal-backdrop';
    modal.innerHTML = `
      <div class="popup-modal">
        <div class="popup-modal-content">
          <span class="popup-modal-message" id="popupModalMsg"></span>
          <div class="popup-modal-actions">
            <button class="popup-modal-btn popup-modal-confirm" id="popupModalConfirm">Yes</button>
            <button class="popup-modal-btn popup-modal-cancel" id="popupModalCancel">No</button>
          </div>
        </div>
      </div>
    `;
  ...
  // Confirm
  document.getElementById('popupModalConfirm').onclick = function() {
    modal.style.display = 'none';
    if (callback) callback(true);
  };
  // Cancel
  document.getElementById('popupModalCancel').onclick = function() {
    modal.style.display = 'none';
    if (callback) callback(false);
  };
}

// --- Feedback Modal Utility (Red for all feedback) ---
function showFeedbackModal(message) {
  ...
  if (!modal) {
    ...
    modal.innerHTML = `
      <div class="popup-modal">
        <div class="popup-modal-content popup-modal-feedback">
          <span class="popup-modal-message" id="feedbackModalMsg"></span>
          <div class="popup-modal-actions">
            <button class="popup-modal-btn popup-modal-close" id="feedbackModalClose">Close</button>
          </div>
        </div>
      </div>
    `;
  ...
  document.getElementById('feedbackModalMsg').textContent = message;
  modal.style.display = 'flex';
  document.getElementById('feedbackModalClose').onclick = function() {
    modal.style.display = 'none';
  };
}
...
function searchStudent() {
  ...
  if (!searchValue) {
    showPopupWarning("Please enter a Student USN or Name");
    return;
  }
  ...
  fetch("/instructor/get_student", {
    ...
    .catch((error) => {
      ...
      showPopupWarning("An error occurred while searching for the student");
    });
}
...
function recommendSubject(code, name, units) {
  ...
  if (recommendedSubjects.some((subject) => subject.code === code)) {
    showPopupWarning(`${code} is already in your recommendations!`);
    return;
  }
...
function sendRecommendations() {
  if (recommendedSubjects.length === 0) {
    showPopupWarning("Please select at least one subject to recommend");
    return;
  }
...
function checkTimeConflicts() {
  ...
  availableSubjects.forEach((row) => {
    ...
    if (hasConflict) {
      ...
      row.classList.add("conflict-row"); // highlight row
      row.classList.remove("success-row"); // remove any previous green
    } else {
...
function recommendSubject(subjectCode, subjectName) {
  if (!currentStudent) {
    showPopupWarning("Please select a student first");
    return;
  }
...
function enlistSubject(subjAvailId, subjectName) {
  if (!selectedStudentId) {
    showFeedbackModal("Please select a student first");
    return;
  }
  ...
  for (let row of enrolledRows) {
    ...
    if (
      ...
      showFeedbackModal("Subject already enrolled.");
      return;
    }
  }

  if (!subjAvailId) {
    showFeedbackModal("Subject ID is missing");
    return;
  }
  ...
  fetch("/instructor/enlist_subject", {
    ...
    .then((data) => {
      if (data.success) {
        showFeedbackModal(`Successfully enlisted in ${subjectName}`);
        fetchStudentDetails(selectedStudentId);

        // Highlight the row green for a short time
        const rows = document.querySelectorAll("#availableSubjects tr");
        rows.forEach((row) => {
          // Assume subjAvailId is somewhere in the row (e.g., as data attribute or code cell)
          if (
            row.innerHTML.includes(`"${subjAvailId}"`) ||
            row.innerHTML.includes(`'${subjAvailId}'`)
          ) {
            ...
            row.classList.remove("conflict-row");
            setTimeout(() => row.classList.remove("success-row"), 2500); // green for 2.5s
          }
        });
      } else {
        showFeedbackModal(data.message || "Failed to enlist subject");
      }
    })
    .catch((error) => {
      console.error("Error:", error);
      showFeedbackModal("Error enlisting subject");
    });
}
  ...
  showPopupModal(
    ...
    function (confirmed) {
      ...
      fetch(`/drop_subject/${id}`, {
        ...
        .then((data) => {
          if (data.success) {
            showFeedbackModal("Subject dropped successfully");
            ...
            showFeedbackModal(data.message || "Failed to drop subject");
          }
        })
        .catch((error) => {
          console.error("Error:", error);
...
function searchStudent() {
  ...
  if (!searchValue) {
    showPopupWarning("Please enter a student name.");
    return;
  }
  ...
  fetch("/instructor/get_student", {
    ...
    .catch((error) => {
      ...
      showPopupWarning("An error occurred while fetching student data.");
    });
}
...
function filterStudents(searchTerm) {
  ...
  fetch("/instructor/search_students", {
    ...
    .catch((error) => {
      console.error("Error:", error);
      showPopupWarning("Error searching for students");
    });
}
...
function fetchStudentDetails(usn) {
  ...
  fetch("/instructor/get_student", {
    ...
    .catch((error) => {
      console.error("Error:", error);
      showPopupWarning("Error loading student details: " + error.message);
    });
}
...
function showStudentInfo(student) {
  ...
  document.getElementById("studentDetails").textContent = student.year
